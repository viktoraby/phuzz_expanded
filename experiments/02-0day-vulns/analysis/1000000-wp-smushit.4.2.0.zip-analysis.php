...
*Overview: {'ajax_handle_track_request': {'smush_analytics_track_event'}, 'reset': {'reset_settings'}, 'ajax_skip_media_hub_connect': {'skip_media_hub_connect'}, 'directory_smush_cancel': {'directory_smush_cancel'}, 'directory_smush_start': {'directory_smush_start'}, 'directory_list': {'smush_get_directory_list'}, 'image_list': {'image_list'}, 'ajax_track_deactivation_survey': {'smush_track_deactivate'}, 'get_dir_smush_stats': {'get_dir_smush_stats'}, ' ! is_array( $to_smush ) ) {\n\t\t\t$to_smush = array();\n\t\t}\n\n\t\treturn array_map(\n\t\t\tfunction ( $image_id ) {\n\t\t\t\treturn new Smush_Background_Task(\n\t\t\t\t\tSmush_Background_Task::get_task_type_smush(),\n\t\t\t\t\t$image_id\n\t\t\t\t);\n\t\t\t},\n\t\t\t$to_smush\n\t\t);\n\t}\n\n\tprivate function prepare_resmush_tasks() {\n\t\t$to_resmush = $this->global_stats->get_redo_ids();\n\n\t\treturn array_map(\n\t\t\tfunction ( $image_id ) {\n\t\t\t\treturn new Smush_Background_Task(\n\t\t\t\t\tSmush_Background_Task::get_task_type_resmush(),\n\t\t\t\t\t$image_id\n\t\t\t\t);\n\t\t\t},\n\t\t\t$to_resmush\n\t\t);\n\t}\n\n\tprivate function prepare_error_tasks() {\n\t\t$error_items_to_retry = $this->global_stats->get_error_list()->get_ids();\n\n\t\treturn array_map(\n\t\t\tfunction ( $image_id ) {\n\t\t\t\treturn new Smush_Background_Task(\n\t\t\t\t\tSmush_Background_Task::get_task_type_error(),\n\t\t\t\t\t$image_id\n\t\t\t\t);\n\t\t\t},\n\t\t\t$error_items_to_retry\n\t\t);\n\t}\n\n\tpublic function localize_background_stats( $script_data, $page_slug ) {\n\t\t$script_data[': {'$action'}, 'directory_smush_finish': {'directory_smush_finish'}, 'process_actions': {'wdev_logger_action'}, 'wp-smush-ajax': {'$action'}, 'directory_smush_check_step': {'directory_smush_check_step'}, 'save_settings': {'smush_save_settings'}}
*
***/
