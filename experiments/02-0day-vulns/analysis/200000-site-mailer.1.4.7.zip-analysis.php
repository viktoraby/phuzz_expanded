...
function handle_deactivation_feedback(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'site_mailer_deactivation_feedback' ) ) {
			wp_send_json_error( esc_html__( 'Security check failed', 'site-mailer' ) );
			return;
		}

		// Check user capabilities
		if ( ! Utils::user_is_admin() ) {
			wp_send_json_error( esc_html__( 'Insufficient permissions', 'site-mailer' ) );
			return;
		}
